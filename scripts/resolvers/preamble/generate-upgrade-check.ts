...
export function generateUpgradeCheck(ctx: TemplateContext): string {
  ...
  const includes = hostConfig.generation.includeSkills;
  const hasUpgradeSkill = !includes?.length || includes.includes('gstack-upgrade');
  const upgradeInstruction = hasUpgradeSkill
    ...
    : 'If output shows `UPGRADE_AVAILABLE <old> <new>`: do not run inline upgrade. Report it and update this fork through Git when the user asks for a gstack refresh.';
