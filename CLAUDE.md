...
**Namleh fork secret rule.** Do not store API keys in `.env` files, shell startup
files, launch agents, generated logs, or worktree setup hooks. Namleh secrets live
in the hosted Vault or in the approved auth store for the tool that needs them.
For paid evals, inject `ANTHROPIC_API_KEY` only for the command process from the
approved secret source for that environment, and never echo it to stdout, logs, or
shell history.

When passing to a test's Agent SDK, do NOT pass `env: {...}` to `runAgentSdkTest`
— the SDK's auth pipeline doesn't pick up the key the same way when env is
supplied as an object (confirmed failure mode). Instead, mutate
`process.env.ANTHROPIC_API_KEY` ambiently before the call and restore in `finally`.
