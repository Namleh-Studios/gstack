...
const CLAUDE_SKIPPED_SKILL_DIRS = new Set([
  'claude',
  ...ALL_SKILLS.filter(skill => skill.dir.startsWith('namleh-')).map(skill => skill.dir),
]);
...
describe('Codex generation (--host codex)', () => {
  ...
  const CODEX_SKILLS = (() => {
    ...
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      if (entry.name.startsWith('namleh-')) continue;
...
describe('plan-mode-info resolver (handshake-replacement)', () => {
  ...
  test('vestigial handshake is absent from non-Claude host outputs when present on disk', () => {
    ...
    const hostDirs = ['.agents', '.openclaw', '.opencode', '.factory', '.hermes', '.kiro', '.cursor', '.slate', '.namleh-gstack'];
    let checked = 0;
